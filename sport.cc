...
void create_user(const string& nama, int harga, const string& kategori, const string& merek, int stok) {
    ...
    if (conn) {
        stringstream query;
        query << "INSERT INTO umam_sport (nama, kategori, merek, harga, stok) VALUES ('" << nama << "', '" << kategori << "', '" << merek << "', " << harga << ", " << stok << ")";
...
void get_users() {
    MYSQL* conn = connect_db();
    if (conn) {
        if (mysql_query(conn, "SELECT * FROM umam_sport")) {
...
void update_user(int user_id, const string& nama, const string& kategori, const string& merek, int harga, int stok) {
    ...
    if (conn) {
        stringstream query;
        query << "UPDATE umam_sport SET nama = '" << nama << "', kategori = '" << kategori << "', merek = '" << merek << "', harga = " << harga << "', stok = "<<stok << " WHERE id = " << user_id;
        if (mysql_query(conn, query.str().c_str())) {
...
void delete_user(int user_id) {
    ...
    if (conn) {
        stringstream query;
        query << "DELETE FROM umam_sport WHERE id = " << user_id;
        if (mysql_query(conn, query.str().c_str())) {
...
void tampilan() {
    ...
    while (true) {
        cout << "====================================\n";
        cout << RED << "Toko raket Berkah Jaya\n" << RESET;
        cout << "====================================\n";
        cout << "\nMenu:\n";
